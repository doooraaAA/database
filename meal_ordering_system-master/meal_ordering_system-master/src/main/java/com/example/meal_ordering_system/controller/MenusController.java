...
//  跳转至修改页面
    ...
    public String toUpdatePage(Integer id,Model model){
        ...
        return "/server/menus_update";
    }
    @RequestMapping("/toUpdatePageadmin")
    public String toUpdatePageadmin(Integer id,Model model){
        Menus menus = menusService.queryById(id);
        model.addAttribute("menu",menus);
        List<Types> typesList=typesService.queryAll();
        model.addAttribute("typesList",typesList);
...
// 插入一个菜单信息到数据库
    ...
    public void update(Menus menus,HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
        System.out.println(menus.getPrice());
        System.out.println(menus.getStatus());
        menusService.update(menus);
        request.getRequestDispatcher("../menus/allMenus").forward(request,response);

    }
    @RequestMapping("/updateadmin")
    public void updateadmin(Menus menus,HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
        System.out.println(menus.getPrice());
        System.out.println(menus.getStatus());
        menusService.update(menus);
        request.getRequestDispatcher("../menus/allMenusadmin").forward(request,response);
    ...
    public void admindelete(Integer id,HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
        menusService.deleteById(id);
        request.getRequestDispatcher("../menus/allMenusadmin").forward(request,response);
    }
