...
public class OrdersController {
    ...
    @RequestMapping("orderchangestatus")
    public String orderchangestatus( HttpServletRequest request) {
        //传给前端
        Pages page1=null;
        //总页面
        Integer totalCount=null;
        ...
        List<Orders> list= ordersService.pageListByDelivery(0);
        request.setAttribute("list",list);

        System.out.println("pagess="+pagess);
        System.out.println("currentpages="+currentpages);

        return "/server/orderchangestatus";
    }
    ...
    public String OrderComfirm(HttpServletRequest request){
        ...
        if(reqtype.equals("delivery")){
            ...
            String TableNo=ordersService.getTableNo(id);
            System.out.println(TableNo);
            int Status = tableService.getTableStatus(TableNo);
            if(Status==1) {
                tableService.updateTableStatus(TableNo, 0);
                Integer userid = usersService.selectbytable(TableNo);
        ...
        return "redirect:/orders/orderchangestatus";
    }
    ...
    public String order_search( HttpServletRequest request) {
        ...
        List<Orders> list= ordersService.pageListByID(totalCount);
        request.setAttribute("list",list);
    ...
    public String  order_qiantai( HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        String day1=null;
        String userid =null;
        if(request.getParameter("userid")!=null &&(!request.getParameter("userid").equals(""))&&!request.getParameter("userid").isEmpty()){
            System.out.println("userid 值为: " + request.getParameter("userid")); // 输出 id 的值
            id=Integer.parseInt(request.getParameter("userid"));
            System.out.println("userid 值为: " + id); // 输出 id 的值
    ...
    @RequestMapping(value="ordergetmyall")
    public String ordergetmyall(HttpServletRequest request,@Param("id")Integer id) throws UnsupportedEncodingException {
    ...
    public String order_addshoppingcartoOrder(HttpServletRequest request) {
        ...
        if(session.getAttribute("user_session")!=null) {
            ...
            Integer id = null;
            String tableNo = usersService.selectusertable(userid);
            System.out.println(tableNo);
            if (shoppingCarts != null) {
                for (ShoppingCart shoppingCart : shoppingCarts) {
                    Integer sums = shoppingCart.getSums();
                    Integer menuid = shoppingCart.getId();
                    Orders orders = new Orders(id, userid, menuid, sums, day, delivery,tableNo);
