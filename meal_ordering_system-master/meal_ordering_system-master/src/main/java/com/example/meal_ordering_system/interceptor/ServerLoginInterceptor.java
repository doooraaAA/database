package com.example.meal_ordering_system.interceptor;//package com.example.meal_ordering_system.interceptor;
...
public class ServerLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if (uri.indexOf("/server/login")>=0) {
            return true;
        }
        //获取session
        HttpSession session = request.getSession();
        Server server = (Server) session.getAttribute("server_session");
        ...
        request.getRequestDispatcher("/public/server/index.jsp").forward(request, response);
        return false;
    }
